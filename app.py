...
# Your original code functions (unchanged, copied here for completeness)
    ...
    for _, row in hospitals.iterrows():
        if row.geometry.geom_type in ["Point", "Polygon"]:
            ...
            try:
                ...
                print(f"Added hospital node {node} at {lat}, {lon}")
            except Exception as e:
                print(f"Error mapping hospital at {lat}, {lon}: {e}")
                continue
...
def get_location_coordinates(place_name):
    geolocator = Nominatim(user_agent="emergency_route_finder")
...
def compute_emergency_route(accident_site):
    ...
    if not hospital_times:
        print("No accessible hospitals found.")
    ...
    print(f"Nearest hospital time: {nearest_time}")
    nearest_path = reconstruct_path(predecessors, accident_node, nearest_hospital)
    if not nearest_path:
        print("No valid path to the nearest hospital.")
    ...
    print(f"Initial path traffic: {initial_traffic_percentage:.1f}% high traffic ({initial_traffic_status})")
    ...
    if nearest_path:
        ...
        folium.PolyLine(path_coords, color="red", weight=5, opacity=0.8, tooltip=initial_traffic_status).add_to(m)
    optimal_path = nearest_path
    optimal_hospital = nearest_hospital
    optimal_time = nearest_time
    optimal_traffic_status = initial_traffic_status
    if initial_traffic_percentage > 30 and len(hospital_times) > 1:
        ...
        if alternative_path:
            alternative_traffic_percentage, alternative_traffic_status = analyze_traffic_path(G, alternative_path, traffic)
            print(f"Alternative path traffic: {alternative_traffic_percentage:.1f}% high traffic ({alternative_traffic_status})")
            if alternative_traffic_percentage <= 30:
                optimal_path = alternative_path
                optimal_hospital = alternative_hospital
                optimal_time = alternative_time
                optimal_traffic_status = alternative_traffic_status
                lat, lon = hospital_coords[optimal_hospital]
                folium.Marker((lat, lon), icon=folium.Icon(color="green"), 
                             popup=f"{optimal_traffic_status}: {optimal_time*3600:.1f}s").add_to(m)
                path_coords = [(G.nodes[node]['y'], G.nodes[node]['x']) for node in optimal_path]
                folium.PolyLine(path_coords, color="green", weight=5, opacity=0.8, tooltip=optimal_traffic_status).add_to(m)
                print("Switched to alternative path with lower traffic.")
            else:
                print("Alternative path also has high traffic. Using initial path.")
        else:
            print("No valid alternative path found. Using initial path.")
    else:
        print("Initial path is optimal due to low traffic or no better alternative.")
    try:
        map_html = m._repr_html_()  # Convert Folium map to HTML string
        print("Map HTML generated successfully.")
    except Exception as e:
        print(f"Error generating map HTML: {e}")
        return "Error generating map", str(e)
    return optimal_traffic_status, map_html
